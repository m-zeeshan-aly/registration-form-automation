...
class RegistrationPage:
    ...
    # 1. Store the XPATH strings as Class Constants
    SUBJECTS_XPATH = "//input[@id='subjectsInput']"
    FILE_UPLOAD_XPATH = "//input[@id='uploadPicture']"
    ...
    @staticmethod
    def get_input(page, placeholder_text):
        return page.locator(RegistrationPage.DYNAMIC_FIELD_XPATH_INPUT.format(label=placeholder_text))
    
    @staticmethod
    def get_radio(page, value):
        return page.locator(RegistrationPage.DYNAMIC_FIELD_XPATH_RADIO.format(value=value))
    @staticmethod
    def get_checkbox(page, label):
        return page.locator(RegistrationPage.DYNAMIC_CHECKBOX_BY_LABEL.format(label=label))
    @staticmethod
    def get_dropdown_container(page, dropdown_id):
        return page.locator(RegistrationPage.DYNAMIC_DROPDOWN_CONTAINER.format(id_name=dropdown_id))
    @staticmethod
    def get_dropdown_option(page, option_text):
        return page.locator(RegistrationPage.DYNAMIC_DROPDOWN_OPTION.format(option_text=option_text))
    @staticmethod
    def get_date_input(page):
        return page.locator(RegistrationPage.DATE_INPUT)
    @staticmethod
    def get_date_select(page, type):
        return page.locator(RegistrationPage.DATE_SELECT_TEMPLATE.format(type=type))
    @staticmethod
    def get_day_locator(page, day):
        formatted_day = str(day).zfill(3)
        return page.locator(RegistrationPage.DAY_TEMPLATE.format(day=formatted_day))
    @staticmethod
    def get_subjects_input(page):
        return page.locator(RegistrationPage.SUBJECTS_XPATH)
    @staticmethod
    def get_file_upload(page):
        return page.locator(RegistrationPage.FILE_UPLOAD_XPATH)
    @staticmethod
    def get_SUBMIT_BUTTON(page):
        return page.locator(RegistrationPage.SUBMIT_BTN_XPATH)
    ...
    @staticmethod
    def type_text(page, locator, value):
    ...
    @staticmethod
    def check_hobby(page, hobby_name):
        locator = RegistrationPage.get_checkbox(page, hobby_name)
        expect(locator).to_be_visible()
    ...
    @staticmethod
    def select_gender(page, gender_name):
        locator = RegistrationPage.get_radio(page, gender_name)
        locator.check()
        return locator

    def verify_date_ui(page, day, month, year):
        ...
        actual_date_value = RegistrationPage.page.locator(RegistrationPage.DATE_INPUT).input_value()
    ...
    def select_dob(page, day, month, year):
        # 1. Open Calendar
        RegistrationPage.get_date_input(page).click()

        # 2. Select Month & Year
        # Note: We call .select_option() on the LOCATOR returned by our getter
        RegistrationPage.get_date_select(page, "month").select_option(label=month)
        RegistrationPage.get_date_select(page, "year").select_option(value=str(year))

        # 3. Select Day
        RegistrationPage.get_day_locator(page,day).click()

    @staticmethod
    def fill_form(page, fname, lname, email, phone_num , gender=None, subjects=None, hobbies=[], address=None, day=None, month=None, year=None, file_path=None, file_name=None, state=None, city=None, use_keyboard=None):
      
        expect(RegistrationPage.type_text(page, RegistrationPage.get_input(page, "First Name"), fname)).to_have_value(fname)
        expect(RegistrationPage.type_text(page, RegistrationPage.get_input(page, "Last Name"), lname)).to_have_value(lname)
        expect(RegistrationPage.type_text(page, RegistrationPage.get_input(page, "name@example.com"), email)).to_have_value(email)
        expect(RegistrationPage.type_text(page, RegistrationPage.get_input(page, "Mobile Number"), phone_num)).to_have_value(phone_num)

        expect(RegistrationPage.select_gender(page, "Male")).to_be_checked()
        expect(RegistrationPage.select_gender(page, "Other")).to_be_checked()
        expect(RegistrationPage.select_gender(page, "Female")).to_be_checked()
        expect(RegistrationPage.select_gender(page, gender)).to_be_checked()
        

        # This method takes a list of hobbies and checks the corresponding checkboxes
        # .check_checkboxs(hobbies)  # Example of checking multiple checkboxes
        for hobby in hobbies:
            checkbox_locator = expect(RegistrationPage.get_checkbox(page, hobby)).to_be_visible()
            expect(RegistrationPage.check_hobby(page, hobby)).to_be_checked()

        # Subjects (Handles the auto-suggest input)
        expect(RegistrationPage.get_subjects_input(page)).to_be_visible()
        RegistrationPage.get_subjects_input(page).fill(subjects)
        page.keyboard.press("Enter")
        expect(RegistrationPage.get_subjects_input(page)).to_have_value("")  # After selection, the input should
        
        # Current Address (Handles textarea automatically!)
        expect(RegistrationPage.type_text(page, RegistrationPage.get_input(page, "Current Address"), address)).to_have_value(address)

        # Selecting date of birth using the method we defined
        RegistrationPage.select_dob(page, day, month, year)  # Example of selecting a date of birth
        # 4. Verify
        # RegistrationPage.verify_date_ui(page, day, month, year)
